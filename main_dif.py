...
class NVMLightningModule(LightningModule):
    ...
    def _common_step(self, batch, batch_idx, stage: Optional[str] = "evaluation"):
        ...
        if self.phase == "ctproj":    
            ...
            # figure_ct_latent = torch.randn_like(image2d) * 0.5 + 0.5  
            # figure_ct_latent_random = figure_ct_latent.clone()
            # figure_ct_latent_hidden = figure_ct_latent.clone()
            # figure_ct_interp_random = self.ddpmsch.add_noise(original_samples=figure_ct_random, noise=figure_ct_latent_random, timesteps=timesteps) 
            # figure_ct_interp_hidden = self.ddpmsch.add_noise(original_samples=figure_ct_hidden, noise=figure_ct_latent_hidden, timesteps=timesteps) 
            ...
            im2d_loss_dif = self.loss(figure_xr_output_hidden.float(), figure_xr_target_hidden.float()) \
                          + self.loss(figure_ct_output_random.float(), figure_ct_target_random.float()) \
                          + self.loss(figure_ct_output_hidden.float(), figure_ct_target_hidden.float()) \
